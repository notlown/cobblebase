...
import notlown.cobblebase.fabric.client.BuildJobStatusCache
...
class BuilderPanel(
    ...
    fun init(addWidget: (ClickableWidget) -> Unit) {
        ...
        if (!dataRequested || BuilderCache.templates.isEmpty()) {
            ClientPlayNetworking.send(StructureTemplateListRequestC2SPacket())
            dataRequested = true
        }
    ...
    fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (pasture != null && nowMs - lastStatusPollMs > STATUS_POLL_INTERVAL_MS) {
            ...
            try {
                ClientPlayNetworking.send(BuildJobStatusRequestC2SPacket(pasture))
    ...
    private fun handleHelperToggleClick(mouseX: Double, mouseY: Double): Boolean {
        ...
        for (mon in pokemonList) {
            ...
            if (mouseX >= x && mouseX <= x + HELPER_ICON_W) {
                ...
                ClientPlayNetworking.send(SkillAssignmentC2SPacket(mon.pokemonId, newAssignment ?: ""))
    ...
    private fun clearActiveJob() {
        val pasture = pastureOrigin ?: return
        ClientPlayNetworking.send(notlown.cobblebase.core.net.BuildJobConfigureC2SPacket(
            pasturePos = pasture,
            templateId = "",
